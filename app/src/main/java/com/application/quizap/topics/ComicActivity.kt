...
class ComicActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreComic.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarComic.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("comicPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreComic.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionComic)
        btnOptions.gravity= Gravity.CENTER_HORIZONTAL
        loadQuestion()
        btnComicNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scoreComicQuestions.text = scoreQuestion.toString()
            scoreComicTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreComicQuestions.text.toString().toInt()>=10) {
                if (scoreComicTotal.text.toString().toInt() > highScore) {
                    highScore = scoreComicTotal.text.toString().toInt()
                    highScoreComic.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnComicQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchComicsQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        comicQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@ComicActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreComicAnswers.text = scoreAnswer.toString()
        scoreComicTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}