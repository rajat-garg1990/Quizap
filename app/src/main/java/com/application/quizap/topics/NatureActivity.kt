...
class NatureActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreNature.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarNature.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("naturePref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreNature.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionNature)
        btnOptions.gravity= Gravity.CENTER_HORIZONTAL
        loadQuestion()
        btnNatureNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scoreNatureQuestions.text = scoreQuestion.toString()
            scoreNatureTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreNatureQuestions.text.toString().toInt()>=10) {
                if (scoreNatureTotal.text.toString().toInt() > highScore) {
                    highScore = scoreNatureTotal.text.toString().toInt()
                    highScoreNature.text = highScore.toString()
    ...
    private fun loadQuestion() {
        ...
        api.fetchNatureQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        natureQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@NatureActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreNatureAnswers.text = scoreAnswer.toString()
        scoreNatureTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}