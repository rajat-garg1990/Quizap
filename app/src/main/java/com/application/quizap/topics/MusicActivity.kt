...
class MusicActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreMusic.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarMusic.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("musicPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreMusic.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionMusic)
        ...
        btnMusicNext.setOnClickListener {
            ...
            loadQuestion()
            scoreQuestion += 1
            scoreMusicQuestions.text = scoreQuestion.toString()
            scoreMusicTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreMusicQuestions.text.toString().toInt()>=10) {
                if (scoreMusicTotal.text.toString().toInt() > highScore) {
                    highScore = scoreMusicTotal.text.toString().toInt()
                    highScoreMusic.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnMusicQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchMusicQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        musicQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@MusicActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreMusicAnswers.text = scoreAnswer.toString()
        scoreMusicTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}