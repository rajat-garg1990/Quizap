...
class PoliticsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScorePolitics.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarPolitics.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("polPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScorePolitics.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionPolitics)
        btnOptions.gravity= Gravity.CENTER_HORIZONTAL
        loadQuestion()
        btnPoliticsNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scorePoliticsQuestions.text = scoreQuestion.toString()
            scorePoliticsTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scorePoliticsQuestions.text.toString().toInt()>=10) {
                if (scorePoliticsTotal.text.toString().toInt() > highScore) {
                    highScore = scorePoliticsTotal.text.toString().toInt()
                    highScorePolitics.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnPoliticsQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchPoliticsQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        politicQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@PoliticsActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scorePoliticsAnswers.text = scoreAnswer.toString()
        scorePoliticsTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}