...
class FilmActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreFilm.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarFilm.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("filmPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreFilm.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionFilm)
        btnOptions.gravity= Gravity.CENTER_HORIZONTAL
        loadQuestion()
        btnFilmNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scoreFilmQuestions.text = scoreQuestion.toString()
            scoreFilmTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreFilmQuestions.text.toString().toInt()>=10) {
                if (scoreFilmTotal.text.toString().toInt() > highScore) {
                    highScore = scoreFilmTotal.text.toString().toInt()
                    highScoreFilm.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnFilmQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchFilmQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        filmQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@FilmActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreFilmAnswers.text = scoreAnswer.toString()
        scoreFilmTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}