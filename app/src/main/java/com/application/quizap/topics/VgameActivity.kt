...
import kotlinx.android.synthetic.main.activity_vgame.*
...
class VgameActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreVg.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarVg.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("vgPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreVg.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionVg)
        btnOptions.gravity= Gravity.CENTER_HORIZONTAL
        loadQuestion()
        btnVgNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scoreVgQuestions.text = scoreQuestion.toString()
            scoreVgTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreVgQuestions.text.toString().toInt()>=10) {
                if (scoreVgTotal.text.toString().toInt() > highScore) {
                    highScore = scoreVgTotal.text.toString().toInt()
                    highScoreVg.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnVgQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchVideoGameQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        vgQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@VgameActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreVgAnswers.text = scoreAnswer.toString()
        scoreVgTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}