...
class AnimeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreAnime.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarAnime.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("animePref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreAnime.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionAnime)
        ...
        btnAnimeNext.setOnClickListener {
            ...
            loadQuestion()
            scoreQuestion += 1
            scoreAnimeQuestions.text = scoreQuestion.toString()
            scoreAnimeTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreAnimeQuestions.text.toString().toInt()>=10) {
                if (scoreAnimeTotal.text.toString().toInt() > highScore) {
                    highScore = scoreAnimeTotal.text.toString().toInt()
                    highScoreAnime.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnAnimeQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchAnimeQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        animeQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@AnimeActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreAnimeAnswers.text = scoreAnswer.toString()
        scoreAnimeTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}