...
class MathsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreMath.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarMath.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("mathPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreMath.text=highScore.toString()
        ...
        btnTvNext.setOnClickListener {
            ...
            loadQuestion()
            scoreQuestion += 1
            scoreMathQuestions.text = scoreQuestion.toString()
            scoreMathTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreMathQuestions.text.toString().toInt()>=10) {
                if (scoreMathTotal.text.toString().toInt() > highScore) {
                    highScore = scoreMathTotal.text.toString().toInt()
                    highScoreMath.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnMathQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchMathematicsQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        mathQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@MathsActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreMathAnswers.text = scoreAnswer.toString()
        scoreMathTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}