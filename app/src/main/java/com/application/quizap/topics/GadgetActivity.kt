...
import kotlinx.android.synthetic.main.activity_gadget.*
...
class GadgetActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreGadget.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarGadget.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("gadgetPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreGadget.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionGadget)
        btnOptions.gravity= Gravity.CENTER_HORIZONTAL
        loadQuestion()
        btnGadgetNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scoreGadgetQuestions.text = scoreQuestion.toString()
            scoreGadgetTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreGadgetQuestions.text.toString().toInt()>=10) {
                if (scoreGadgetTotal.text.toString().toInt() > highScore) {
                    highScore = scoreGadgetTotal.text.toString().toInt()
                    highScoreGadget.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnGadgetQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchGadgetQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        gadgetQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@GadgetActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreGadgetAnswers.text = scoreAnswer.toString()
        scoreGadgetTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}