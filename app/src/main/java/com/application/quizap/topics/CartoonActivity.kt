...
class CartoonActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreCartoon.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarCartoon.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("cartoonPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreCartoon.text=highScore.toString()
        ...
        btnTvNext.setOnClickListener {
            ...
            loadQuestion()
            scoreQuestion += 1
            scoreCartoonQuestions.text = scoreQuestion.toString()
            scoreCartoonTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreCartoonQuestions.text.toString().toInt()>=10) {
                if (scoreCartoonTotal.text.toString().toInt() > highScore) {
                    highScore = scoreCartoonTotal.text.toString().toInt()
                    highScoreCartoon.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnCartoonQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchCartoonQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        cartoonQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@CartoonActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreCartoonAnswers.text = scoreAnswer.toString()
        scoreCartoonTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}