...
import kotlinx.android.synthetic.main.activity_bgame.*
import kotlinx.android.synthetic.main.activity_geography.*
import kotlinx.android.synthetic.main.activity_geography.scoreT4
...
class GeographyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreGeo.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarGeo.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("geoPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreGeo.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionGeo)
        btnOptions.gravity= Gravity.CENTER_HORIZONTAL
        loadQuestion()
        btnGeoNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scoreGeoQuestions.text = scoreQuestion.toString()
            scoreGeoTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreGeoQuestions.text.toString().toInt()>=10) {
                if (scoreGeoTotal.text.toString().toInt() > highScore) {
                    highScore = scoreGeoTotal.text.toString().toInt()
                    highScoreGeo.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnGeoQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchGeographyQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        geoQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@GeographyActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreGeoAnswers.text = scoreAnswer.toString()
        scoreGeoTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}