...
class HistoryActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreHistory.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarHistory.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("tvPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreHistory.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionHistory)
        ...
        btnHistoryNext.setOnClickListener {
            ...
            loadQuestion()
            scoreQuestion += 1
            scoreHistoryQuestions.text = scoreQuestion.toString()
            scoreHistoryTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreHistoryQuestions.text.toString().toInt()>=10) {
                if (scoreHistoryTotal.text.toString().toInt() > highScore) {
                    highScore = scoreHistoryTotal.text.toString().toInt()
                    highScoreHistory.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnHistoryQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchHistoryQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        historyQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@HistoryActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreHistoryAnswers.text = scoreAnswer.toString()
        scoreHistoryTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}