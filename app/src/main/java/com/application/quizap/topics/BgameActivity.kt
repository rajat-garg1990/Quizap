...
class BgameActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (auth.currentUser==null) {
            highScoreBg.visibility = View.GONE
            scoreT4.visibility = View.GONE
        }
        topBarBg.setNavigationOnClickListener { finish() }
        val prefs= getSharedPreferences("bgPref", Context.MODE_PRIVATE)
        val edit = prefs.edit()
        highScore=prefs.getInt("highScore",0)
        highScoreBg.text=highScore.toString()
        btnOptions=findViewById(R.id.cgOptionBg)
        btnOptions.gravity= Gravity.CENTER_HORIZONTAL
        loadQuestion()
        btnBgNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scoreBgQuestions.text = scoreQuestion.toString()
            scoreBgTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
            if (scoreBgQuestions.text.toString().toInt()>=10) {
                if (scoreBgTotal.text.toString().toInt() > highScore) {
                    highScore = scoreBgTotal.text.toString().toInt()
                    highScoreBg.text = highScore.toString()
                    edit.putInt("highScore", highScore).apply()
                }
            }
        }
        btnBgQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchBoardGameQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        bgQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@BgameActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreBgAnswers.text = scoreAnswer.toString()
        scoreBgTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}