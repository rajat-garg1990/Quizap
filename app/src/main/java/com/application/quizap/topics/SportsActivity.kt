...
class SportsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loadQuestion()
        btnSportsNext.setOnClickListener {
            btnOptions.removeAllViewsInLayout()
            loadQuestion()
            scoreQuestion += 1
            scoreSportsQuestions.text = scoreQuestion.toString()
            scoreSportsTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
        ...
        btnSportsQuit.setOnClickListener {
    ...
    private fun loadQuestion() {
        ...
        api.fetchSportQuestion().enqueue(object : Callback<QuestionResponse> {
            ...
            override fun onResponse(
                ...
                if (result != null) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        sportsQuestion.text = HtmlCompat.fromHtml(
                    ...
                    options.shuffle()
                    for (a in options) {
                        var chip = Button(this@SportsActivity)
                        chip.text =
    ...
    private fun checkAnswer(correct: String, chip: Button) {
        ...
        scoreSportsAnswers.text = scoreAnswer.toString()
        scoreSportsTotal.text = ((scoreAnswer * 100)/ scoreQuestion) .toString()
    }
}